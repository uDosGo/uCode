//! Grid parser implementation

use super::{GridDocument, GridCell, GridComponent};

/// Parse a simple ASCII grid into a grid document
pub fn parse_ascii_grid(title: &str, grid: &[&str]) -> GridDocument {
    let rows = grid.len();
    if rows == 0 {
        return GridDocument::new(title, 0, 0);
    }
    let cols = grid[0].len();

    let mut doc = GridDocument::new(title, rows, cols);
...
/// Parse a grid document with components
pub fn parse_with_components(title: &str, grid: &[&str], components: &[GridComponent]) -> GridDocument {
...
/// Parse a simple grid format from text
/// Format:
/// title: My Grid
/// rows: 3
/// cols: 3
...
/// DEF
/// GHI
pub fn parse_simple_grid(text: &str) -> Result<GridDocument, String> {
...
mod tests {
    ...
    fn test_parse_ascii_grid() {
        ...
        let doc = parse_ascii_grid("Test Grid", &grid);
    ...
    fn test_parse_with_components() {
        ...
        let doc = parse_with_components("Test Grid", &grid, &components);
...
DEF";
        ...
        let doc = parse_simple_grid(text).unwrap();
    ...
    #[test]
    fn test_parse_simple_grid_errors() {
        // Test invalid rows
        let text1 = 
"title: Test
rows: abc
...
ABC
DEF";
        assert!(parse_simple_grid(text1).is_err());
...
ABC
DEF";
        assert!(parse_simple_grid(text2).is_err());
...
ABC
DEFG";
        assert!(parse_simple_grid(text3).is_err());
    }
}
