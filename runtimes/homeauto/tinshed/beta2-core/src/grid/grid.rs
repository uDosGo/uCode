...
use super::{GridDocument, GridCell};
use std::collections::HashMap;

/// Grid operations for grid documents
pub trait GridOperations {
    ...
    /// Get subgrid
    fn get_subgrid(&self, row: usize, col: usize, height: usize, width: usize) -> Result<GridDocument, String>;
...
impl GridOperations for GridDocument {
    fn to_string_representation(&self) -> String {
    ...
    fn get_subgrid(&self, row: usize, col: usize, height: usize, width: usize) -> Result<GridDocument, String> {
        ...
        let mut subgrid_doc = GridDocument::new(
...
mod tests {
    ...
    fn test_to_string_representation() {
        let mut doc = GridDocument::new("Test", 2, 3);
        
        let mut cell1 = GridCell::default();
        cell1.char = 'A';
        doc.set_cell(0, 0, cell1).unwrap();
        
        let mut cell2 = GridCell::default();
        cell2.char = 'B';
        doc.set_cell(0, 1, cell2).unwrap();
        
        let mut cell3 = GridCell::default();
        cell3.char = 'C';
    ...
    fn test_find_cells_by_char() {
        let mut doc = GridDocument::new("Test", 3, 3);
    ...
    fn test_count_char_occurrences() {
        let mut doc = GridDocument::new("Test", 2, 2);
        
        let mut cell_a = GridCell::default();
        cell_a.char = 'A';
        doc.set_cell(0, 0, cell_a.clone()).unwrap();
        doc.set_cell(0, 1, cell_a.clone()).unwrap();
        
        let mut cell_b = GridCell::default();
        cell_b.char = 'B';
    ...
    #[test]
    fn test_get_subgrid() {
        let mut doc = GridDocument::new("Test", 4, 4);
    ...
    fn test_get_subgrid_out_of_bounds() {
        let doc = GridDocument::new("Test", 2, 2);
