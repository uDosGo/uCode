...
### AMOS Sprite & BOB Extensions

uCode1 includes an AMOS-to-BBCSDL compatibility shim (`core_py/bbc/amos_shim.bbc`) that provides Amiga-style sprite, BOB (Blitter Object), collision, and multimedia commands for retro game development.

**Usage:** `INSTALL "core_py/bbc/amos_shim.bbc"`

#### Sprite Commands

| Command | Syntax | Description |
|---------|--------|-------------|
| `SPRITE` | `SPRITE id, x, y, bank` | Create or move a hardware sprite (16px, AMOS-style) |
| `MOVE` | `MOVE id, x, y, steps` | Animate sprite movement over N steps |
| `AUTOPATH` | `AUTOPATH id, x, y` | Set automatic pathfinding for sprite |

#### BOB Commands (Blitter Objects)

| Command | Syntax | Description |
|---------|--------|-------------|
| `BOB` | `BOB id, x, y, gif_file$` | Create a BOB from an animated GIF |
| `BOB_OFF` | `BOB_OFF id` | Hide a BOB |
| `BOB_UPDATE` | `BOB_UPDATE` | Update all BOB animations |

#### Collision Detection

| Command | Syntax | Description |
|---------|--------|-------------|
| `=COLL` | `=COLL(id1, id2)` | Returns TRUE if two sprites collide |
| `=ZONE` | `=ZONE(id, x1, y1, x2, y2)` | Returns TRUE if sprite is within zone |

#### Sound & Screen

| Command | Syntax | Description |
|---------|--------|-------------|
| `SOUND` | `SOUND ch, vol, pitch, dur` | Play a sound effect |
| `MUSIC` | `MUSIC filename$` | Play background music |
| `MUSIC_OFF` | `MUSIC_OFF` | Stop background music |
| `SCREEN` | `SCREEN mode` | Set screen mode (0=320×256 8-bit, 1=640×512 2-colour, 2=640×512 4-colour, 3=640×512 16-colour, 13=320×256 32-bit) |
| `WAIT` | `WAIT frames` | Wait for N frames (50Hz = 20ms per frame) |

#### Example

```basic
10 INSTALL "core_py/bbc/amos_shim.bbc"
20 MODE 1
30 SPRITE 1, 100, 50, 1
40 MOVE 1, 300, 200, 10
50 IF =COLL(1, 2) THEN PRINT "Hit!"
60 SOUND 1, 100, 200, 50
```
